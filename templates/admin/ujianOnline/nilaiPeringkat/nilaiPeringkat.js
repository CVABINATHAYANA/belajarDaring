angular.module('app.nilaiPeringkatUjianOnlineAdmin', [])

    .controller('nilaiPeringkatUjianOnlineAdminCtrl', ['$scope', '$stateParams', '$firebaseArray', '$firebaseObject', '$ionicPopup', '$ionicLoading', '$state', '$ionicModal', '$ionicActionSheet', '$timeout', '$filter', function ($scope, $stateParams, $firebaseArray, $firebaseObject, $ionicPopup, $ionicLoading, $state, $ionicModal, $ionicActionSheet, $timeout, $filter) {
        ...
        var ref = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa").orderByChild("idUjian").equalTo($scope.data.idUjian);
        var listRef = $firebaseArray(ref);
        ...
        listRef.$loaded().then(function (response) {
            ...
            if ($scope.data.totalSiswaLolos !== "") {
                for (i = 0; i < $scope.peringkat.length; i++) {
                    if (i < $scope.data.totalSiswaLolos) {
                        var updateLulus = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.peringkat[i].$id);
                        updateLulus.update({
                            "lulus": true,
                        }).then(function (resp) {
                            console.log('lulus');
        ...
        $scope.dataNilaiSiswa = function (data) {
            //console.log(data);
            $state.go("menuAdmin.nilaiSiswaUjianOnlineAdmin", {
                "idUjian": $stateParams.idUjian,
                "namaUjian": $stateParams.namaUjian,
                "jenjang": $stateParams.jenjang,
                "namaKotaKabupaten": $stateParams.namaKotaKabupaten,
    ...
    .controller('nilaiSiswaUjianOnlineAdminCtrl', ['$scope', '$stateParams', '$firebaseArray', '$firebaseObject', '$ionicPopup', '$ionicLoading', '$state', '$ionicModal', '$ionicActionSheet', '$timeout', '$filter', function ($scope, $stateParams, $firebaseArray, $firebaseObject, $ionicPopup, $ionicLoading, $state, $ionicModal, $ionicActionSheet, $timeout, $filter) {
        ...
        var ref = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.data.idRekapJawabanUjianOnlineSiswa);
        ...
        var refNilai = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.data.idRekapJawabanUjianOnlineSiswa + "/pelajaran");
        ...
        listRefNilai.$loaded().then(function (response) {
            for (i = 0; i < response.length; i++) {
                if (response[i].nilai !== undefined) {
                    ...
                    var updateNilaiRata = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.data.idRekapJawabanUjianOnlineSiswa);
        ...
        $scope.goToNilai = function (data) {
            ...
            $ionicActionSheet.show({
                ...
                buttonClicked: function (index) {
                    if (index === 0) {
                        var getData = firebase.database(appUjianOnline).ref("jawabanUjianOnlineSiswaPerPelajaran").orderByChild("filter").equalTo($stateParams.idUjian + "_" + data.idPelajaranUjianOnline + "_" + $scope.data.uid);
                        getData.on("child_added", function (snapshot) {
                            //console.log(snapshot.val());
                            $state.go("menuAdmin.nilaiSiswaDetailUjianOnlineAdmin", {
                                "idUjian": $stateParams.idUjian,
                                "namaUjian": $stateParams.namaUjian,
                                "jenjang": $stateParams.jenjang,
                                "namaKotaKabupaten": $stateParams.namaKotaKabupaten,
                ...
                destructiveButtonClicked: function () {
                    var refNilai = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.data.idRekapJawabanUjianOnlineSiswa + "/pelajaran/" + data.$id);
                    ...
                    confirmPopup.then(function (resp) {
                        if (resp) {
                            nilaiHapus.$remove().then(function (response) {
                                var avgNilai = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.data.idRekapJawabanOlimpiadeSiswa + "/pelajaran");
                                ...
                                listAvgNilai.$loaded().then(function (response) {
                                    for (i = 0; i < response.length; i++) {
                                        if (response[i].nilai !== undefined) {
                                            ...
                                            // console.log("total Nilai", $scope.nilaiTotal);
                                            // console.log("avg", $scope.avg);
                                            var updateNilaiRata = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.data.idRekapJawabanOlimpiadeSiswa);
        ...
        $scope.printNilai = function () {
            ...
            if ($scope.jenjang === "SMP") {
                ...
                doc.text(200, 230, 'Admin, ', null, null, 'right');
    ...
    .controller('nilaiSiswaDetailUjianOnlineAdminCtrl', ['$scope', '$stateParams', '$firebaseArray', '$firebaseObject', '$ionicPopup', '$ionicLoading', '$state', '$ionicModal', '$ionicActionSheet', '$timeout', '$filter', function ($scope, $stateParams, $firebaseArray, $firebaseObject, $ionicPopup, $ionicLoading, $state, $ionicModal, $ionicActionSheet, $timeout, $filter) {
        ...
        if ($scope.data.statusFinish === false) {
            ...
            var refPengaturanUmum = firebase.database(appUjianOnline).ref("pengaturanUmumUjianOnline").orderByChild("filter").equalTo($stateParams.idUjian + "_" + $stateParams.idPelajaranUjianOnline);
            ...
            var hitungNilai = firebase.database(appUjianOnline).ref("jawabanUjianOnlineSiswa").orderByChild("filter").equalTo($stateParams.idUjian + "_" + $stateParams.idPelajaranUjianOnline + "_" + $scope.data.uid);
            ...
            listHitungNilai.$loaded().then(function (response) {
                ...
                //Lock Ujian
                var ref = firebase.database(appUjianOnline).ref("jawabanUjianOnlineSiswaPerPelajaran/" + $scope.data.idJawabanOlimpiadeSiswaPerPelajaran);
        ...
        if ($scope.data.statusFinish === true) {
            var ref = firebase.database(appUjianOnline).ref("jawabanUjianOnlineSiswaPerPelajaran/" + $scope.data.idJawabanUjianOnlineSiswaPerPelajaran);
        ...
        var avgNilai = firebase.database(appUjianOnline).ref("rekapJawabanUjianOnlineSiswa/" + $scope.data.idRekapJawabanUjianOnlineSiswa + "/pelajaran");
        var listAvgNilai = $firebaseArray(avgNilai);
    ...
    .controller('statusJawabanSiswaUjianOnlineAdminCtrl', ['$scope', '$stateParams', '$firebaseArray', '$firebaseObject', '$ionicPopup', '$ionicLoading', '$state', '$ionicModal', '$ionicActionSheet', '$timeout', '$filter', function ($scope, $stateParams, $firebaseArray, $firebaseObject, $ionicPopup, $ionicLoading, $state, $ionicModal, $ionicActionSheet, $timeout, $filter) {
        ...
        if ($scope.data.keterangan === "all" || $scope.data.keterangan === "Kosong") {
            var ref = firebase.database(appUjianOnline).ref("jawabanUjianOnlineSiswa").orderByChild("filter").equalTo($scope.data.idUjian + "_" + $scope.data.idPelajaranUjianOnline + "_" + $scope.data.uid);
            var listRef = $firebaseArray(ref);
            ...
            var ref = firebase.database(appUjianOnline).ref("jawabanUjianOnlineSiswa").orderByChild("filterStatusJawaban").equalTo($stateParams.idUjian + "_" + $stateParams.idPelajaranUjianOnline + "_" + $scope.data.uid + "_Benar");
            var listRef = $firebaseArray(ref);
            ...
            var ref = firebase.database(appUjianOnline).ref("jawabanUjianOnlineSiswa").orderByChild("filterStatusJawaban").equalTo($stateParams.idUjian + "_" + $stateParams.idPelajaranUjianOnline + "_" + $scope.data.uid + "_Salah");
            var listRef = $firebaseArray(ref);
        ...
        $scope.getSoal = function (jawaban) {
            ...
            var pembahasanSoal = firebase.database(appUjianOnline).ref("pengaturanUmumUjianOnline").orderByChild("filter").equalTo($stateParams.idUjian + "_" + $stateParams.idPelajaranUjianOnline);
