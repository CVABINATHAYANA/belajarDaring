angular.module('app.berandaLatihanSoalOnlineSiswa', [])

    .controller('berandaLatihanSoalOnlineSiswaCtrl', ['$scope', '$stateParams', '$firebaseArray', '$firebaseObject', '$ionicPopup', '$ionicLoading', '$state', '$ionicModal', '$ionicActionSheet', '$timeout', '$filter', function ($scope, $stateParams, $firebaseArray, $firebaseObject, $ionicPopup, $ionicLoading, $state, $ionicModal, $ionicActionSheet, $timeout, $filter) {
        ...
        list.$loaded().then(function (response) {
            ...
            if (response.length === 0) {
                ...
                for (i = 0; i < response.length; i++) {
                    ...
                    var refUjianOnine = firebase.database(app).ref("ujianOnline/" + idUjian);

                    refUjianOnine.on("value", function (snapshot) {
                        $ionicLoading.hide();
                        $scope.namaUjian = snapshot.val();
                        // console.log($scope.namaUjian);
                        if($scope.namaUjian.ruangLingkupUjian==="Sekolah" && $scope.namaUjian.jenisUjian!=="Ulangan Harian"){
        ...
        // $scope.untukUmum = function () {
        //     $ionicLoading.show();
        //     var refSiswa = firebase.database().ref("dataSiswa/" + $scope.idPenggunaSiswa);
        //     refSiswa.on("value", function (snapshot) {
        //         $scope.tingkatKelas = snapshot.val().kelas;

        //         var reflatihanSoal = firebase.database().ref("ujianOnline").orderByChild("filter").equalTo("Latihan Soal Online_Umum" + "_" + $scope.tingkatKelas);
